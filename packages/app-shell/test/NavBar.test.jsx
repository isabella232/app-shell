...
import { act, fireEvent, leftClick, render, screen } from '@testing-library/react';
import { UserContext } from '../src/index';
import { account as mockAccount } from './mocks/account';
import NavBar from '../src/NavBar';
import { QUERY_ACCOUNT, SET_CURRENT_ORGANIZATION } from '../src/graphql/account';
...
function renderNavBarWithAccount(account) {
  render(
    <MockedProvider mocks={[{
      ...
      <UserContext.Provider value={account}><NavBar/></UserContext.Provider>
    </MockedProvider>,
  );
}
...
describe('NavBar', () => {
  describe('Organization switcher', () => {
    ...
    it ('does not render org switcher if there is only one organization', async () => {
      ...
      const foundOrganization = await screen.queryByText('My only organization')
